...
class ToolHook
{
    ...
    {
        try {
            if ($table == 'tx_rkwtools_domain_model_tool') {
                DebuggerUtility::var_dump($value); exit;
                // if is file, set it to typolink (for frontend handling in Partial Tool/List/Boxes/Default.html)
                if ($fieldArray['link_to_file']) {

                }
            }

        } catch (\Exception $e) {
            $this->getLogger()->log(\TYPO3\CMS\Core\Log\LogLevel::ERROR, sprintf('Cannot convert fileReference to typolink. Reason: %s', $e->getMessage()));
        }
    ...
    public function processDatamap_postProcessFieldArray($status, $table, $id, &$fieldArray, &$reference)
    {
        try {
            ...
            // 3.) Handle typolink, if resource is a file
            if (
                ($table == 'tx_rkwtools_domain_model_tool')
                && (is_int($id))
                && ($record = BackendUtility::getRecord('tx_rkwtools_domain_model_tool', $id))
                ...
                if (
                    ...
                    // get file id of reference
                    $sysFileReferenceResult = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                        "uid_local",
                        "sys_file_reference",
                        "tablenames = 'tx_rkwtools_domain_model_tool' AND uid_foreign = '" . $record['uid'] . "'"
                    );

                    // we only have one file. In case of some logical changes, the last entry would win
                    $sysFileUid = 0;
                    foreach ($sysFileReferenceResult as $sysFileReference) {
                        $sysFileUid = $sysFileReference['uid_local'];
                    }

                    if ($sysFileUid) {
                        // add to fieldArray, to persist entry
                        $fieldArray['link'] = 'file:' . $sysFileUid;
                    }
                }
            }
